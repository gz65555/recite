...
export async function addNoteBook(ctx: Context | any, next) {
  const user = ctx.user;
  ctx.body = await service.notebook.createNotebook(ctx.request.body.name, user.uid);
  await next()
}

export async function deleteNotebook(ctx: Context | any, next) {
  const user = ctx.user;
  ctx.body = await service.notebook.deleteNotebook(ctx.params.id, user.uid);
  await next()
}

export async function updateNotebook(ctx: Context | any, next) {
  const user = ctx.user;
...
export async function getAllNotebook(ctx: Context | any, next) {
  const user = ctx.user;
